...
A lightweight, modular operating system built from the ground up on **Cosmos Gen3** using the Native AOT patcher. VoidOS has transitioned from a GUI-focused environment into a robust, server-oriented architecture, low-level networking, custom TCP protocols, and a highly extensible command-line interface.
